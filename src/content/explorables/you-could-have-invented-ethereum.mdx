...
<Scene name="blocks" caption="A real block header — number, parentHash, txsRoot, stateRoot — over the trie whose root is that stateRoot. Edit one balance and watch it ripple: leaf → new state root → header → block hash → every following block." />
