...
<Scene name="dbdiff" caption="Each layer holds only the keys its block touched. A read falls from the newest layer down and takes the first match — so block N's A=9 shadows the stale A=7 below it. Miss every layer and you read the full base." />
...
<Scene name="dbmerge" caption="Merge = union the two diffs, keep the newest value per key. A was written in both blocks, so the older A=7 is dropped and only A=9 survives — three entries instead of four. The merged layer is smaller than the two it replaced." />
