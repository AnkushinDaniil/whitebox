...
summary: The single, complete explanation of how Ethereum works — a step-by-step, fully-animated derivation from scratch (signatures, accounts, the Merkle-Patricia trie, the hash-linked blockchain, proof-of-stake consensus, the EVM, and gas), each piece introduced exactly when the last one's flaw demands it, with the real differentially-tested engines woven into the steps and the spec detail behind each. Original analogies, adapted to Ethereum.
topic: Start here
order: -1
spine: true
...
<JourneyNav client:load steps={[
  ...
  { id: "step-5", label: "One fingerprint" },
  { id: "step-6", label: "The blockchain" },
  { id: "step-7", label: "Agreeing on one chain" },
  { id: "step-8", label: "Cells that compute" },
  { id: "step-9", label: "Metering work" },
]} />
...
<Scene name="verify" caption="The same signature, two messages. The untouched one verifies (✓ valid, signer = you); flip one byte and verification fails (✗ invalid) — there is no in-between." />
...
<WidgetFrame
  ...
  caption="Any input, however long, becomes exactly 32 bytes. Change one character and almost every output byte flips. That sensitivity is the entire reason a fingerprint can detect change."
...
<Formula
  expr="private key → public key → address"
  terms={[
    { t: "private key", note: "a random 256-bit secret — the only thing you keep" },
    { t: "public key", note: "secp256k1 scalar-multiplies it onto the curve (one-way)" },
    { t: "address", note: "the last 20 bytes of keccak256(public key)" },
  ]}
  caption="Each arrow goes one way only — right, never left. That asymmetry is exactly what a signature proves."
/>
...
<Callout type="warning" title="What breaks">
Everyone holds the full sheet — but it's hundreds of millions of accounts, and copies drift. How does a node prove, in one cheap check, that its copy is the *exact same world* as everyone else's, without shipping the entire sheet around to compare it row by row?
</Callout>
...
<DepthLayer tier="mechanism" id="step-5" num="5" badge="step" title="One fingerprint">
...
<Scene name="root" caption="Edit an account, re-hash up its path, and a brand-new 32-byte state root falls out — the single number that fingerprints the whole world." />
...
<WidgetFrame
  ...
  caption="Insert or edit an account. Nodes glide to new positions and the changed path pulses up to a brand-new state root. Click a node to inspect its RLP and hash."
...
<Callout type="warning" title="What breaks">
One number now pins down *what the balances are* — but it's a snapshot of right-now. It says nothing about the *order* edits happened in, and nothing stops someone quietly rewriting a past edit and recomputing a fresh-looking root. The history itself still needs to be ordered and made impossible to rewrite.
</Callout>
...
<DepthLayer tier="mechanism" id="step-6" num="6" badge="step" title="The blockchain">
...
<DepthLayer tier="mechanism" id="step-7" num="7" badge="step" title="Agreeing on one chain">
...
<DepthLayer tier="mechanism" id="step-8" num="8" badge="step" title="Cells that compute">
...
<DepthLayer tier="mechanism" id="step-9" num="9" badge="step" title="Metering work">
...
A programmable, decentralized, metered world computer, secured by staked money, every account folded into one verifiable number. Read the chain of fixes backward and nothing in it was arbitrary: **signatures** because edits could be forged; **recovery** because authorship had to be provable, not claimed; the **nonce** and **chainId** because a signature alone is endlessly replayable; **decentralization** because one host is a bank; the **Merkle-Patricia trie** and its **state root** because every node must agree on the whole world at a glance — and prove any part of it cheaply; the **hash-linked blockchain** because that state still needs an ordered, un-rewritable history; **proof-of-stake consensus** because competing chains need a costly, punishable vote to converge; the **EVM** and its **data** field because numbers can't hold rules; and **gas** with its **limit** and **fee** fields because code that can do anything must not be able to run forever for free. Not one field, not one mechanism, was a free choice — each was forced by the flaw in the one before it. You could have invented every piece.
</Callout>
